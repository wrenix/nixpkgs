            ...
            if [ -f "$prg" ]; then
              ...
              if [ -x "$prg" ]; then
                if [ -f ".$prg-wrapped" ]; then
                  ...
                  makeWrapper "$path/bin/$prg" "$out/bin/$prg" --inherit-argv0-path ${lib.optionalString (!permitUserSite) ''--set PYTHONNOUSERSITE "true"''} ${lib.concatStringsSep " " makeWrapperArgs}
                fi
              fi
            fi
          done
